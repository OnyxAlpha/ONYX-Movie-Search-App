...
export default function Header () {
  ...
  return (
    <header>
          <nav className=" md:grid-cols-4 bg-white border-b-2 border-gray-200 px-3 lg:px-8 py-2 dark:bg-gray-900 dark:border-b-1 dark:border-gray-900">
            <div className="flex flex-wrap justify-between items-center mx-auto px-0">
              <Link to="/" className="flex items-center">
                  ...
                  <span className=" text-xl nav-title self-center pt-1 lg:text-3xl lg:block font-semibold whitespace-nowrap dark:text-white">OnyxCinema</span>
              </Link>
              <div id='mobile-nav' className="flex md:order-2">

               {/* Dark mode Button */}
                <button onClick={() => setDarkMode(!darkMode)} data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" type="button" aria-label='Dark and light mode' data-toggle-dark="light" className="flex items-center p-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-500 dark:bg-gray-800 focus:outline-none dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                  {/* svg */}
                ...
                <button onClick={() => setHiddenSearch(!hiddenSearch)} type="button" data-collapse-toggle="navbar-search" aria-label='Search' aria-controls="navbar-search" aria-expanded="false" className="md:hidden text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5 mr-1">
                ...
                <div className="relative hidden md:block">
                  ...
                  <form onSubmit={handleSubmit}>
                    <input type="text" name="search" id="search-navbar-mobile" className="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search..." autoComplete='off' />
                  </form>
                </div>
              ...
              {/* ----- */}
              {/* ----- */}
                <div id='nav-links' className={`${hiddenSearch ? 'hidden' : ''} items-center justify-between w-full md:flex md:w-auto md:order-1"} id="navbar-search`}>
                  ...
                  <ul className="flex flex-col p-4 pt-5 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row lg:space-x-8 md:space-x-2 md:mt-0 text-xl md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                    <li>
                      <NavLink to="/allmovies" onClick={() => setHiddenSearch(!hiddenSearch)} className={ ({isActive}) => isActive ? activeClass : inactiveClass } end><span className="md:text-lg lg:text-2xl">Movies</span></NavLink>
                    </li>
                    <li>
                      <NavLink to="/alltvshows" onClick={() => setHiddenSearch(!hiddenSearch)} className={ ({isActive}) => isActive ? activeClass : inactiveClass }><span className="md:text-lg lg:text-2xl">TV Shows</span></NavLink>
                    </li>
                    <li>
                      <NavLink to="/signin" onClick={() => setHiddenSearch(!hiddenSearch)} className={ ({isActive}) => isActive ? activeClass : inactiveClass }><span className="md:text-lg lg:text-2xl">Sign In</span></NavLink>
                    </li>
                    {/* <li>
                    ...
                    {/* <li>
                      ...
                      <NavLink to="/" onClick={() => setHiddenSearch(!hiddenSearch)} className={ ({isActive}) => isActive ? activeClass : inactiveClass }><span className="md:text-lg lg:text-2xl">TV Shows</span></NavLink>
                    </li> */}
                  </ul>
                </div>
              </div>
